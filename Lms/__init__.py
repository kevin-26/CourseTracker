...
time.sleep(3)
a = WebDriverWait(driver, timeout = 10).until(expected_conditions.visibility_of_element_located((By.ID, "paging-control-limit-container-1")))
a.find_element_by_tag_name("button").click()
a.find_elements_by_tag_name('a')[2].click()
lms_s = login.getCourseList(driver)
lms = []
for i in lms_s:
    lms.append(subject.getSubject(driver, i[0]))